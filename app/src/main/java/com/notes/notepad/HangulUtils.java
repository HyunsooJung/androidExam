...
public class HangulUtils {
    ...
    //getIsChoSungList함수에서 불린값 처리후 getHangulInitialSound로 리턴
    public static String getHangulInitialSound(String value,
    ...
    /**
     * 유니코드 이용해 초성검색 모든 글자에 초성이 들어가는게 있는지 확인하기위해 유니코드와 들어온 데이터 비교
     * @param value
     * @param isChoList
     * @return
     */
    public static String getHangulInitialSound(String value, boolean[] isChoList) {
        ...
        for (int idx = 0; idx < unicodeList.length; idx++) {
            ...
            Log.v("unicode : ","unicode : "+unicode);
            Log.v("isChoList.length : ","isChoList.length : "+isChoList.length);
            if (isChoList != null ) {
               // Log.v("isCholist : ","isCholist : "+isChoList[idx]);
                if (true) {
                    if (HANGUL_BEGIN_UNICODE <= unicode
                            && unicode <= HANGUL_END_UNICODE) {
                        int tmp = (unicode - HANGUL_BEGIN_UNICODE);
                        int index = tmp / HANGUL_BASE_UNIT;
                        result.append(INITIAL_SOUND[index]);
                        Log.v("result : ","result : "+result.toString());
                    } else {
                        result.append(convertUnicodeToChar(unicode));
                        Log.v("result1 : ","result1 : "+result.toString());
                    }
                } else {
                    result.append(convertUnicodeToChar(unicode));
                    Log.v("result2 : ","result2 : "+result.toString());
                }
            } else {
                result.append(convertUnicodeToChar(unicode));
                Log.v("result3 : ","result3 : "+result.toString());
            }
        }
        Log.v("result4 : ","result4 : "+result.toString());
        return result.toString();
